...
abstract class Kernel extends QMatrix {
    ...
    public static Map<String, Integer> class2id ;
    public static Map<Integer, String> id2class ;
    public final static String URI = "http://phenomebrowser.net/smltest/" ;
    public static URIFactory factory = URIFactoryMemory.getSingleton();
    static URI graph_uri = factory.getURI(URI);
    static G graph = null;
    static GDataConf graphconf = null;
    public static SM_Engine engine = null ;
	...
	switch(kernel_type)
	    ...
	    case svm_parameter.SEMANTIC:
		return evalSim(x[i], x[j]) ;
	    default:
		return 0;	// java
	    }
    }

    static double evalSim(svm_node[] x, svm_node[] y) {
	//System.out.println(param.ontology_file);
	Set<URI> s1 = new LinkedHashSet<URI>() ;
	Set<URI> s2 = new LinkedHashSet<URI>() ;
	for (int i = 0 ; i < x.length ; i++) {
	    s1.add(factory.getURI(x[i].classURI)) ;
	}
	...
	try {
	icConf = new IC_Conf_Topo("Intrinsic term specificity measure", SMConstants.FLAG_ICI_HARISPE_2012);                                                       
	...
	if (param.ontology_file != null) { // set up the graph for SML
	    initSimilarityEngine(param.ontology_file) ;
	}
	this.kernel_type = param.kernel_type;
	this.degree = param.degree;
	this.gamma = param.gamma;
	this.coef0 = param.coef0;
	this.class2id = param.class2id ;
	this.id2class = new HashMap() ;
	for (String s : class2id.keySet()) {
	    Integer ind = class2id.get(s) ;
	    id2class.put(ind, s) ;
	}
...
public class svm {
    ...
    public static void initSimilarityEngine(String ontfile) {
	Kernel.initSimilarityEngine(ontfile);
    }
    ...
    {
	"linear","polynomial","rbf","sigmoid","precomputed","semantic"
    };
	...
	for(int i=0;i<l;i++)
		...
		for(int j=0;j<n;j++)
			...
			String classURI = id2class.get(index) ;
			model.SV[i][j].index = index;
			model.SV[i][j].value = value;
			model.SV[i][j].classURI = classURI ;
		    }
	    }
